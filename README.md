...
**贡献者**：本仓库用 `develop` 作为集成/预览主干，并有 pre-push 敏感信息扫描钩子。clone 后请先启用一次：
...
- **工程**：GitHub → EdgeOne 多环境自动部署（main=生产 / 其余=预览）· pre-push 密钥扫描钩子 · develop 集成主干分支模型。
