...
关联 GitHub 后，EdgeOne Pages 是**默认多环境**的：一个项目下同时挂着「生产」和「预览」两套独立部署，
在**控制台 → 环境管理**里可见。二者并存、互不覆盖。

### 环境规则（控制台实测）

| 环境 | 域名 | 关联分支 | 自动部署 |
|---|---|---|---|
| **生产** | `topic.yeranyang.com` | `main`（独占） | 开启 |
| **预览** | 自动分配 `xxxxx.edgeone.app` | **所有未分配环境的 Git 分支** | 开启 |

- `main` 归生产环境独占；**除 `main` 外任何分支** push 后，自动落入**预览环境**并构建，分配独立预览 URL。
- 预览与生产**完全隔离**，官方保证「主站点不受影响」。无需手动登记分支名——预览规则就是「所有未分配环境的 Git 分支」。

### 关键：构建配置随分支走，不互相污染

EdgeOne 的构建配置**优先读代码仓库里的 `edgeone.json`**，而它随分支各读各的：

- `main` 读自己的 `edgeone.json`（`outputDirectory: ./`，静态站，无构建）
- `feature/*` 读自己的（`outputDirectory: ./dist` + Vite 构建 + `copy-functions`）

因此可以「生产跑旧静态版、预览跑新 SaaS 架构」，两套配置零冲突。这也是为什么升级架构应**先开分支**而非直接改 `main`。

### 唯一共享资源：KV

`TOPIC_KV` 是**项目级**资源，生产与预览读写的是**同一个命名空间**。

- 好处：预览环境不用重新灌种子，直接读现有 31 篇。
- 风险：在预览站调 `/api/admin/seed` 等**写操作**会同时改到生产站读到的数据。验证预览时应**只读不写**，或另建独立 KV。

### 升级架构的推荐流程

1. 开 `feature/xxx` 分支，在分支内改 `edgeone.json` / 构建脚本。
2. push → 自动进预览环境构建 → 拿 `.edgeone.app` 预览 URL 验证。
3. 预览逐端点验证通过后，再合并 `main` → 触发生产部署。
4. 全程 `main` 生产站保持可演示，零风险。
